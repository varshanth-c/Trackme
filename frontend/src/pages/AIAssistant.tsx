...
export default function AIAssistant() {
  ...
  const handleSendMessage = useCallback(async (question: string, rerunSql?: string) => {
  ...
  }, [isLoading, user, token, isMobile, fetchHistory, toast]);
...
// --- ChatPanel (defined OUTSIDE AIAssistant to prevent remount on every render) ---
interface ChatPanelProps {
  messages: Message[];
  isLoading: boolean;
  inputMessage: string;
  setInputMessage: (v: string) => void;
  handleSendMessage: (q: string, sql?: string) => void;
  messagesEndRef: React.RefObject<HTMLDivElement>;
  isMobile: boolean;
  setMobileView: (v: 'chat' | 'history') => void;
}
    ...
    <Card className="h-[85vh] lg:h-[80vh] flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2">
          <Bot className="w-5 h-5 text-primary" /> NL-to-SQL Assistant
        </CardTitle>
        {isMobile && (
...
// --- HistoryPanel (defined OUTSIDE AIAssistant to prevent remount on every render) ---
...
function HistoryPanel({
  ...
  return (
    <Card className="h-[85vh] lg:h-[80vh] flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2">
          <History className="w-5 h-5 text-primary" /> Query History
        </CardTitle>
        {isMobile && (
            ...
            ? history.map(item => (
              <div key={item.id} className="text-sm p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                ...
                <Button
                  ...
                  onClick={() => handleSendMessage(item.question, item.sql_query)}
                  disabled={isLoading}
                >
...
function ChatMessage({ message }: { message: Message }) {
  ...
  return (
    <div className={`flex items-start gap-3 max-w-full ${message.isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      {message.isUser && (
        <div className="flex-shrink-0 p-2 rounded-full bg-primary/10 border">
